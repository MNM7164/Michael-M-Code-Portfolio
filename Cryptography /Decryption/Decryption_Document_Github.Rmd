...
```{r}
#Now were going to do some number theory before we create our next cipher.
...
#The next integer is 2, so we start our test with n = 2.
  ...
  if(a %% b == 0){
    ...
    #Example: 12 and 4, 12 = 3*4 meaning the GCD(12,4) is 4.
...
shift_decrypt<- function(x,k,n){
  ...
  #instead of +k. And that is ok, because were still doing modular arrhythmic.
...
affine_decrypt <- function(x,a,b,n){
  i = inverse_mod(a,n)
  #First and foremost we have to find the inverse of our coefficient. We are working
...
that can be argued as viable options. All of the codes below are going to be loops
that will print out every result for the decryption.

```{r}
shift_brute_force <- function(x,n){
  for(i in 1:n){
    shift_decrypt(x,i,n)
  }
}
...
```{r}
affine_brute_force <- function(x,n){
  for(i in 1:n){
    for(j in 1:n){
      if(coprime(i,n)== TRUE){
        #We have the coprime step simply to optimize our results.
       affine_decrypt(x,i,j,n)
      }
    }
  }
}
